...
%   wavenumber ZKS(1) and ZKS(2), scaled by COEFS, i.e.
...
%   constructs the difference of neumann data for 
%   single-layer Helmholtz kernels with 
%   wavenumber ZKS(1) and ZKS(2), scaled by COEFS, i.e.
%   COEFS(1)*KERNEL.HELM2D('sp', ZKS(1)) -
%   COEFS(2)*KERNEL.HELM2D('sp', ZKS(2));
%
%   KERNEL.HELM2DDIFF('dp', ZKS) or KERNEL.HELM2DDIFF('dprime', ZKS) 
%   constructs the difference of neuamnn data for
%   double-layer Helmholtz kernels with 
%   wavenumber ZKS(1) and ZKS(2), scaled by COEFS, i.e.
...
switch lower(type)
    ...
    case {'dp', 'dprime'}
        ...
        obj.sing = 'hs';
        if(abs(coefs(1)-coefs(2))<eps), obj.sing = 'log'; end
