...
xflam = xflam(:,opts_perm.iperm);
 ...
 tic, Fskel = rskelf(matfun,xflam(:,1:opts_perm.ns(1)),occ,rank_or_tol,pxyfun_rere,opts); toc;
...
function [Kpxy,nbr] = pxy_fun_multi(x,slf,nbr,l,ctr,prxy_pts,ks)

    pxy = [prxy_pts(1,:)*l(1)+ctr(1);prxy_pts(2,:)*l(2)+ctr(2)];
    y = x(:,slf);

    Kpxy = [];
    for i=1:numel(ks)
        k = ks(i);
        [v,grad,hess]=chnk.helm2d.green(k,y,pxy);
        tmat = [v;grad(:,:,1);grad(:,:,2);hess(:,:,1);hess(:,:,2);hess(:,:,3)];
        Kpxy = [Kpxy;tmat];
    end
    Kpxy = Kpxy/(size(Kpxy,2));
end
