...
rank_or_tol = 1e-14;
occ = 400;
width = max(max(chnkrtotal)-min(chnkrtotal));
optsnpxy = []; optsnpxy.rank_or_tol = rank_or_tol;
optsnpxy.nsrc = occ;

type = 'evalg';
zk = max(abs(prxy_ks));
kern = @(srcinfo,targinfo) chnk.helm2d.kern(zk,srcinfo,targinfo,type,1);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 ...
 ifaddtrans = true;
 type = 'evalg';
kern = @(kval,srcinfo,targinfo) chnk.helm2d.kern(kval,srcinfo,targinfo,type,1);
 pxyfun = @(x,slf,nbr,l,ctr) chnk.flam.proxyfun_ell2_multi(slf,...
        nbr,l,ctr,chnkrtotal,wts, ...
        kern,opdims,pr,ptau,pw,pin,ifaddtrans,prxy_ks,opts_perm);
