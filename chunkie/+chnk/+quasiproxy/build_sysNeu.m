function fullsys = build_sysNeu(chnkr,kh1,d,theta,ht,xleft,K)

% parameters that we may want user to have option to pass in the future
nproxy = 160;
relrad=2;
nwall=120; % number of points in each wall segment 
nwall_TD=60; % top and bottom wall for RB expansion

% bloch phase
ima = sqrt(-1);
alpha = exp(ima*kh1*d*cos(theta));

% make neighbors
chnkr_l = chnkr;
chnkr_l.r(1,:) = chnkr_l.r(1,:)-d;
chnkr_r = chnkr;
chnkr_r.r(1,:) = chnkr_r.r(1,:)+d;


%build system

[A0,AL,AR] = build_A_matNeu(chnkr,chnkr_l,chnkr_r,kh1);

Amat = A0 + alpha^(-1)*AL + alpha*AR;

[Bmat,Cproxy1,pw] = make_B_matrix(chnkr,d,nproxy,kh1,relrad);
% 
% plot(Cproxy1.r(1,:),Cproxy1.r(2,:),'rx');

[Cmat,lwall1] = make_C_matrix(chnkr,kh1,alpha,ht,d,nwall,xleft);

% plot(lwall1.r(1,:),lwall1.r(2,:),'k.')


[Dmat] = make_D_matrix(Cproxy1,pw,lwall1,kh1,d,alpha);


[Zmat,uwall] = make_Z_matrix(chnkr,chnkr_l,chnkr_r,kh1,alpha,ht,nwall_TD);

% plot(uwall.r(1,:),uwall.r(2,:),'rx');

Vmat = make_V_matrix(Cproxy1,pw,kh1,uwall);

Wmat = make_W_matrix(kh1,uwall,d,theta,K);


%tristan proposed
Cproxy = cell(3,1);
lwall = cell(1,1);
udwall = cell(1,1);


Cproxy{1} = Cproxy1;
Cproxy{3} = pw;

lwall{1} = lwall1;

udwall{1} = uwall;


fullsys=[];
% add matrices
fullsys.Amat = Amat;
fullsys.Bmat = Bmat;
fullsys.Cmat = Cmat;
fullsys.Dmat = Dmat;
fullsys.Vmat = Vmat;
fullsys.Zmat = Zmat;
fullsys.Wmat = Wmat;
% add proxies
fullsys.Cproxy = Cproxy;
% add geometry information
fullsys.lwall = lwall;
fullsys.udwall = udwall;

% store domain information
fullsys.d = d;
fullsys.ht = ht;

fullsys.khs = [kh1];

% build region labeler
% fullsys.cgrph_lab = build_reg_labeler(chnkr,hb,ht);

return



function [A0,AL,AR] = build_A_matNeu(chnkr,chnkr_l,chnkr_r, kh1)
% A0 = self interaction
% AL = interaction with right neighbor
% AR = interaction with left neighbor

% define the necessary kernels
% I only want the adjoint double layer with kh1
Ak1 = kernel('helm','sp',kh1);

% build A0
% self contributions - continuity of u and it's flux
A0 = chunkermat(chnkr,Ak1);
% % % % what is going on in line 109?
A0 = -1/2*eye(size(A0,2)) + A0;
% build interactions with left neighbor A:
AL = chunkerkernevalmat(chnkr_l, Ak1, chnkr);
% build interactions with right neighbor AR
AR = chunkerkernevalmat(chnkr_r, Ak1, chnkr);
return

function [Bmat,Proxy1,pw] = make_B_matrix(chnkr,d,nproxy,kh1,relrad)
% 
 xc1 = [0.5*(chnkr.rstor(1,end,end)+chnkr.rstor(1,1,1));chnkr.rstor(2,1,1)];

[pr, pnorm, pw] = chnk.flam.proxy_circ_pts(nproxy); 

pw = relrad*d*pw/1.5;

Proxy1 = [];
Proxy1.r = relrad*d*pr/1.5+xc1;
Proxy1.n = pnorm;


% only need the derivative evaluated on the chnkr!
B11 = construct_proxy_flux(kh1,Proxy1,pw,chnkr);

Bmat = B11;

return


function A = construct_proxy_flux(kh, Proxy,pw,chnkr)

repcoef = [1,1i*kh];
A = chnk.helm2d.kern(kh,Proxy,chnkr,'cprime',repcoef).*pw.';

return


function A = construct_proxy_all(kh, Proxy,pw,chnkr)

repcoef = [1,1i*kh];
A = chnk.helm2d.kern(kh,Proxy,chnkr,'c2trans',repcoef).*pw.';

return

function [Cmat,lwall1] = make_C_matrix(chnkr,kh1,alpha,ht,d,n,xval)


% make the wall for top layer
[xs,~,~,~] = lege.exps(n);

% Linear map from[-1,1] to [a,b]
yy=(chnkr.r(2,1)*(1-xs)+ht*(1+xs))/2.';

lwall1 = [];
lwall1.r = [xval*ones(1,n);yy.'];
lwall1.n = [ones(1,n);zeros(1,n)];

    rwall1=lwall1;
    rwall1.r(1,:)=rwall1.r(1,:)+d;

    [matR1,matL1] = make_Cblock(lwall1,rwall1,chnkr,kh1,d);

 Cmat = alpha^(-2)*matR1 -alpha*matL1;

return

function [matR,matL] = make_Cblock(lwall,rwall,chnkr,kh,d)


% define the necessary kernels

lwalltmp = lwall;
lwalltmp.r(1,:,:) = lwalltmp.r(1,:,:)-d;
rwalltmp = rwall;
rwalltmp.r(1,:,:) = rwalltmp.r(1,:,:)+d;

ww = chnkr.wts; ww = ww(:).';


matR = [chnk.helm2d.kern(kh,chnkr,rwalltmp,'s').*ww;...
        chnk.helm2d.kern(kh,chnkr,rwalltmp,'sprime').*ww];
matL = [chnk.helm2d.kern(kh,chnkr,lwalltmp,'s').*ww;...
    chnk.helm2d.kern(kh,chnkr,lwalltmp,'sprime').*ww];

return


function [Dmat] = make_D_matrix(Proxy1,pw,lwall1,kh1,d,alpha)


rwall1 = lwall1;
rwall1.r(1,:) = rwall1.r(1,:)+d;

[DL,DR]=make_Dblock(lwall1,rwall1,Proxy1,pw,kh1);

Dmat =  alpha^(-1)*DR-DL;

return
% 
function [DL,DR]=make_Dblock(lwall,rwall,Proxy,pw,kh)
% - Evaluate operator D in Cho and Barnett (2015)

repcoef = [1,1i*kh];
DR = chnk.helm2d.kern(kh,Proxy,rwall,'c2trans',repcoef).*pw.';
DL = chnk.helm2d.kern(kh,Proxy,lwall,'c2trans',repcoef).*pw.';

return

function [Zmat,uwall] = make_Z_matrix(chnkr,chnkr_l,chnkr_r,kh1,alpha,ht,nwall_TD)

x_lft=-0.5;
xtmp = (1+(2*(1:nwall_TD)/nwall_TD - 1 - 1/nwall_TD))/2;
xpts = (1-xtmp)+x_lft;

uwall = [];
uwall.r = [xpts;ht*ones(1,nwall_TD)];
uwall.n = [zeros(1,nwall_TD);ones(1,nwall_TD)];


% make top
[A0,AL,AR]=make_Zblock(uwall,chnkr,chnkr_l,chnkr_r,kh1);

Zmat = A0+alpha^(-1)*AL + alpha*AR;

return
% 
% 
function [A0,AL,AR]=make_Zblock(xpts,C,CL,CR,kh)
% -Evaluate operator Z in Cho and Barnett (2015)

ww = C.wts; ww = ww(:).';

AL = [chnk.helm2d.kern(kh,CL,xpts,'s').*ww;...
    chnk.helm2d.kern(kh,CL,xpts,'sprime').*ww];
A0 = [chnk.helm2d.kern(kh,C,xpts,'s').*ww;...
    chnk.helm2d.kern(kh,C,xpts,'sprime').*ww];
AR = [chnk.helm2d.kern(kh,CR,xpts,'s').*ww;...
    chnk.helm2d.kern(kh,CR,xpts,'sprime').*ww];

return

function Vmat = make_V_matrix(Proxy1,pw,kh1,uwall)


Vmat = construct_proxy_all(kh1,Proxy1,pw,uwall);

return

function Wmat = make_W_matrix(kh1,uwall,d,theta,K)

nwall_TD = size(uwall.r,2);
ima = sqrt(-1);

kappa = kh1*cos(theta)+2*pi*(-K:1:K)/d;
ktmp = ones(nwall_TD,1)*kappa;

%TODO: change to take positive or negative depending on sign of inside 
ku = ones(nwall_TD,1)*sqrt(kh1^2-kappa.^2);
UU = uwall.r(1,:)'*ones(1,2*K+1);

Wmat = zeros(2*size(UU,1),size(UU,2));
Wmat(1:2:end,:) = -exp(ima*UU.*ktmp);
Wmat(2:2:end,:) = -ima*ku.*exp(ima*UU.*ktmp);


return


function cgrph_lab = build_reg_labeler(chnkr,hb,ht)
% detect left and right ends
vend = chunkends(chnkr); 
[~,il] = min(vend(1,:));
[~,ir] = max(vend(1,:));

lend = vend(:,il);
rend = vend(:,ir);

% build chunkgraph out of chunker, and all of the walls
verts = [lend,rend, [lend(1);ht],[rend(1);ht],[lend(1);hb],[rend(1);hb] ];
edge2vert = [[1;2],[3;4],[5;6],[1;3],[1;5],[2;4],[2;6]];

cgrph_lab = chunkgraph(verts,edge2vert);
cgrph_lab.echnks(1) = chnkr;
cgrph_lab.regions = findregions(cgrph_lab);
return