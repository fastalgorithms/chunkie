% HYPOCT   Build hyperoctree to a uniform depth
%
%    T = HYPOCT_UNI(X,BL) builds a hyperoctree T over a set of
%    points X such that
%    each nonempty hypercube node in T is recursively subdivided until it
%    has side-length no larger than BL.
%    The tree T is structured as follows:
%
%        NLVL  - tree depth
...
function T = hypoct_uni(x,bl,lvlmax,ext)
  ...
  % check inputs
  assert(bl >= 0,'FLAM:hypoct:invalidBl', ...
         'Target box size must be non-negative.')
