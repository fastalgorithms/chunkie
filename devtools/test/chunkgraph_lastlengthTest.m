...
for j = 1:nverts
    loc_edges = cgrph3.vstruc{j}{1};
    loc_dir = cgrph3.vstruc{j}{2};

    nloc = length(loc_edges);
    if nloc == 0, continue, end
    arcs = zeros(1,nloc);

    idch_loc = [cgrph3.echnks(loc_edges).nch];
    idch_loc(loc_dir == -1) = 1;

    for k = 1:nloc
        wts = cgrph3.echnks(loc_edges(k)).wts;
        arcs(k) = sum(wts(:,idch_loc(k)));
        if ifplot
        plot(cgrph3.echnks(loc_edges(k)).r(1,:,idch_loc(k)), cgrph3.echnks(loc_edges(k)).r(2,:,idch_loc(k)),'o')
        end
    end
